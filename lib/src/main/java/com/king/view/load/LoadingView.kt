...
class LoadingView: View {
    ...
    private fun init(context: Context?,attrs: AttributeSet?) {
        ...
        var a = context?.obtainStyledAttributes(attrs,R.styleable.LoadingView)
        ...
        var s = Math.abs(mMaxSpeed - mMinSpeed) * 1.0f / mCount
        var offsets = 360f / mCount
        for(i in 0 until mCount){
            mOffsets[i] = mStartAngle + offsets * i
            mOffsetSpeeds[i] = mMinSpeed + s * i
        }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        var defaultSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,60f,resources.displayMetrics).toInt()

        var width = measureHandler(widthMeasureSpec,defaultSize)
        var height = measureHandler(heightMeasureSpec,defaultSize)

        mCircleX = (width + paddingLeft - paddingRight) / 2.0f
        mCircleY = (height + paddingTop - paddingBottom) / 2.0f

        var padding = Math.max(paddingLeft + paddingRight , paddingTop + paddingBottom)
    ...
    private fun measureHandler(measureSpec: Int, defaultSize: Int): Int{
        ...
        when(measureMode){
            ...
            MeasureSpec.AT_MOST -> result = Math.min(defaultSize,measureSize)
        }
    ...
    private fun drawLoading(canvas: Canvas?){
        ...
        //遍历绘制圆弧
        for(i in 0 until mCount){
            var rectF = RectF(mCircleX - mRadius + i * mCirclePadding,mCircleY - mRadius + i * mCirclePadding, mCircleX + mRadius - i * mCirclePadding, mCircleY + mRadius - i * mCirclePadding)
